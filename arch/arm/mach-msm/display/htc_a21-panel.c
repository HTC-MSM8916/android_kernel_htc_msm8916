...
#define PANEL_ID_A21_TRULY_HX8389 1
...
static int htc_a21_regulator_init(struct platform_device *pdev)
{
	int ret = 0;
	...
	pwrdata->lcm_1pv8_en = of_get_named_gpio(pdev->dev.of_node,"htc,lcm_1pv8_en-gpio", 0);
	if (!gpio_is_valid(pwrdata->lcm_1pv8_en)) {
		pr_err("%s can not get LCM 1v8 gpio config\n",__func__);
		return -EINVAL;
	}
...
static int htc_a21_regulator_deinit(struct platform_device *pdev)
{
...
void htc_a21_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	if (pdata == NULL) {
	...
	if (enable) {
		...
		if (pdata->panel_info.panel_id == PANEL_ID_A21_TRULY_HX8389) {
...
static void htc_a21_bkl_en(struct mdss_panel_data *pdata, int enable)
{
	static int en = 0;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	struct dsi_power_data *pwrdata = NULL;
	...
	if (enable) {
		...
		/* Boost Control */
		/* frequency 1MHz, FMODE = 0, current limit 1.2A, voltage limit 40V */
		val = 0x70;
...
static int htc_a21_panel_power_on(struct mdss_panel_data *pdata, int enable)
{
	int ret;
	...
	if (enable) {
		...
		if (!gpio_is_valid(pwrdata->lcm_1pv8_en)) {
			pr_debug("%s:%d, lcm_1pv8_en line not configured\n",
			__func__, __LINE__);
			return -EINVAL;
		}
		gpio_set_value((pwrdata->lcm_1pv8_en), 1);
		...
		gpio_set_value((pwrdata->lcm_1pv8_en), 0);
		usleep_range(2000,2500);
...
static struct mdss_dsi_pwrctrl dsi_pwrctrl = {
	.dsi_regulator_init = htc_a21_regulator_init,
	.dsi_regulator_deinit = htc_a21_regulator_deinit,
	.dsi_power_on = htc_a21_panel_power_on,
	.dsi_panel_reset = htc_a21_panel_reset,
	.bkl_config = htc_a21_bkl_en,
};
