...
static void batt_level_adjust(unsigned long time_since_last_update_ms)
{
	...
	if (htc_batt_info.rep.level == 0) {
		if (htc_batt_info.rep.batt_vol > 3400 && htc_batt_info.rep.batt_temp > 0) {
			pr_info("[BATT] Not reach shutdown voltage, vol:%d\n", htc_batt_info.rep.batt_vol);
			htc_batt_info.rep.level = 1;
		}
	}

	if (htc_batt_info.rep.level == 1) {
		if (htc_batt_info.rep.batt_vol < 3400 && htc_batt_info.rep.batt_temp > 0 &&
			htc_batt_info.rep.level_raw == 0) {
			pr_info("[BATT] Reach shutdown voltage, vol:%d\n", htc_batt_info.rep.batt_vol);
