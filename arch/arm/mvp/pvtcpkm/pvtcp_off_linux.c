	     ...
	     int inet6)
{
	...
	if (likely(uid_eq(uid, Mvpkm_vmwareUid))) {
		if (sock->type == SOCK_DGRAM) {
			/*
			 * Deny sending to UDP port in pvsock range, if
			 * receiving socket was not created by the guest with
			 * this pvsock address. Drop all other UDP packets.
			 */
...
Inet6NfHook(const struct nf_hook_ops *ops,
	    ...
	    int (*okfn)(struct sk_buff *))
{
	/* Not ipv4-mapped? not a pvsock address. */
...
GetLoopbackAddr(void)
{
	...
	for (idx = 1; idx < PVTCP_OFF_MAX_LB_ADDRS; idx++) {
		if (!PvtcpTestLoopbackBit(loopbackAddrs[idx],
			...
			/*
			 * Create a dgram socket to configure/bring-up the
			 * lo:N interface.
			 */
...
GetNetNamespace(PvtcpState *state)
{
	...
	/* Abstract namespace for AF_UNIX/LOCAL sockets */
			      ...
			      unsigned short port)
{
	...
	if (rc) {
		...
		/*
		 * pvsock data usage shouldn't be counted as MVP
		 * external traffic.
		 */
			      ...
			      unsigned short port)
{
	...
	if (rc) {
		...
		/*
		 * pvsock data usage shouldn't be counted as
		 * MVP external traffic.
		 */
		SkFromPvsk(pvsk)->sk_socket->file = NULL;
	}
			...
			unsigned int *addr)
{
	if (!PvtcpHasSockNamespace(pvsk)) {
		...
		/* If it's a pvsock address but not the host's, overwrite it. */
			...
			struct in6_addr *in6)
{
	if (!PvtcpHasSockNamespace(pvsk) && ipv6_addr_v4mapped(in6)) {
		if (PvskTestFlag(pvsk, PVTCP_OFF_PVSKF_IPV6_LOOP)) {
			/*
			 * If the original address came in as ::1, we reset
			 * as such. */
	      ...
	      PvtcpSock *parentPvsk)
{
	...
	if (!parentPvsk) {
		...
		/*
		 * Copy the parent's saved callbacks. The parent pvsk is only
		 * passed when creating/initializing a socket after an 'accept'.
		 */
	      ...
	      unsigned int vecLen)
{
	...
	if (!rc) {
		 ...
		 * The error code is fixed by the PV side so error codes are
		 * properly reported.
		 */
	       ...
	       unsigned int vecLen)
{
	...
	/*
	 * - remove the pvsk from the state, so the state destructor can't find
	 *   it (the state destructor cannot run while operations are in-flight)
	 * - hold the socket before setting the 'release' flag and until after
	 *   the call to PvtcpSchedSock(): if the socket had already been
	 *   scheduled ReleaseAIO may run, find the flag set and release this
	 *   socket while it's being unlocked here.
	    ...
	    unsigned int vecLen)
{
	...
	/*
	 * The socket-level option SO_REUSEADDR is set in the common socket
	 * code, meaning that we cannot intercept it in the guest pvtcp
	 * implementation.
	 * In order to respect the setting, the guest would pass the current
	 * setting in 'bind' requests.
	 * If the guest requires 'reuse address' setting, the value is
	 * incremented such that we differentiate between: 0) not requested,
	 * 1) 'false' and 2) 'true'.
	 */
		  ...
		  unsigned int vecLen)
{
	...
	if (packet->data32 == SOL_TCP) {
		/*
		 * The back-end implementation must always run in 'nodelay'
		 * mode. Consequently, we ignore, but we cache the TCP_NODELAY
		 * and TCP_CORK settings such that getsockopt() can return them
		 * as they were 'set'.
		 * Applications use these settings for performance; pvtcp does
		 * quite well if it's not interfered with.
		 */
		  ...
		  unsigned int vecLen)
{
	...
	if (packet->data32 == SOL_TCP) {
		/*
		 * See comment in PvtcpSetSockOptOp() regarding special
		 * treatment for the TCP_NODELAY and TCP_CORK settings.
		 */
...
connect:
	...
	/*
	 * Quite likely, stream actual connect requests will set err to
	 * EINPROGRESS. That's fine, error_report will trigger an AIO/flow-op
	 * reply. When the connection is established, state_change schedules
	 * an AIO/connect reply.
...
CreateAIO(PvtcpSock *pvsk)
{
	...
	/*
	 * This is a create-error socket. The error reply has been sent
	 * out already, by PvtcpFlowAIO(). This is a paranoid safety
	 * measure, as PVTCP_OP_CREATE OpFlag should not have been set.
	 */
	if (!sk)
		return;
...
AcceptAIO(PvtcpSock *pvsk)
{
	...
	if (pvsk->peerSockSet) {
		...
		/*
		 * accept() succeeded, so this is the child socket.
		...
		if ((rc != packet.len) && !COMM_OPF_TEST_ERR(packet.flags)) {
			/*
			 * Mustn't leak the new socket if PV can't get
			 * a hold of it.
			 */
...
PvtcpProcessAIO(CommOSWork *arg)
{
	...
	if (!SOCK_OUT_TRYLOCK(pvsk)) {
		 ...
		 * There are only two reasons for not being able to lock:
		 * - IoOp() has it -- when done, it reschedules us if we're
		 *   not running.
	...
	if (!SOCK_IN_TRYLOCK(pvsk)) {
		...
		if (sk && sk->sk_socket) {
			...
			/*
			 * Workqueue handlers are pinned to CPU cores and
			 * therefore not migratable. No need to disable
			 * preemption.
			 */
			...
			if (PvskTestOpFlag(pvsk, PVTCP_OP_RELEASE)) {
				...
				/*
				 * All possible in-flight operations must
				 * be dropped.
				 */
				goto doneInUnlock;
			}
...
doneInUnlock:
			 ...
			 * Note that this socket was created by
			 * SockAllocErrInit() and so no 'real' socket sits
			 * atop it and is not present on any state netif list.
			 * The socket has a refcnt of one and it will get
			 * deallocated by the PvtcpPutSock() call below, so we
