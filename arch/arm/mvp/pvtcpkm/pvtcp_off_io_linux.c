	...
	if (sk->sk_type == SOCK_STREAM) {
		...
		if (!SOCK_OUT_TRYLOCK(pvsk)) {
			if (pvsk->peerSockSet &&
				...
				/* Write directly as many bytes as we can. */

				/*
				 * kernel_sendmsg() may use memcpy_fromiovec()
				 * that "modifies the original iovec".
				 */
				struct kvec *vecTmp =
				...
				if (rc >= 0) {
					...
					/*
					 * We never close offload sockets
					 * unless told by the PV side, or when
					 * the comm goes down. Getting out of
					 * sync with PV sockets is a dangerously
					 * bad idea.
					 * This is very likely an
					 * EPIPE/ECONNRESET.
					 */
			...
			/*
			 * We enqueue the bytes for aio processing. Note that
			 * request level ordering is preserved since we're still
			 * under the dispatch lock. However, accessing 'queue'
			 * must be protected via the state lock to serialize
			 * with aio changes.
			 * Note that the struct socket *sock may have been
			 * released, but here we only access sk which is held
			 * (albeit potentially orphaned).
			 */
...
enqueueBytes:
		...
		if (pvsk->peerSockSet) {
			/*
			 * Flow-control already done, based on
			 * PVTCP_SOCK_SAFE_RCVSIZE, just as with stream sockets.
			 * Meaning that we block the senders in the guest
			 * (if applicable).
			 *
			 * The send buffer size was set high enough, at socket
			 * creation time, to avoid dropping datagrams during
			...
			for (vecOff = 0; vecOff < vecLen; vecOff++) {
				if (vec[vecOff].iov_len == 0) {
					...
					/*
					 * Backup iov_base as it may be modified
					 * by kernel_sendmsg().
					 * New net/ipv4/ping.c is using
					 * memcpy_fromiovec() that "modifies
					 * the original iovec".
					 */
					...
					/*
					 * Fragmented payload containing an
					 * embedded iovec?
					 */
	...
	if (pvsk->err || err) {
		...
		if (CommOS_ReadAtomic(&pvsk->sentSize) >= tmpSize) {
			if ((SkFromPvsk(pvsk)->sk_type != SOCK_STREAM) &&
				...
				/*
				 * Don't send dgram flow op until WriteSpaceCB
				 * tells us to do so.
				 */
...
#define VEC_SIZE 32
	...
	sk = SkFromPvsk(pvsk);
	/* If this is an error socket, we don't process it. */
	if (!sk)
		return;
...
again:
	...
	while (!done && CommOS_ReadAtomic(&pvsk->queueSize) > 0) {
		/*
		 * Note: only stream sockets can have a positive send queue
		 * size. Similar to PvtcpIoOp: we must check if sock
		 * (struct socket *) is still valid.
		 */
		...
		if (sk->sk_state == TCP_ESTABLISHED) {
			...
			if (rc >= 0) {
				/*
				 * If we wrote anything, dispose of the buffers
				 * in question.
				 */
				...
				if (!CommOS_ListEmpty(&queue)) {
					/*
					 * Add the remaining bytes to the
				...
				/*
				 * Very likely, this is due to the socket
				 * being closed, so fine.
				 */
			...
			/*
			 * Dispose of all buffers in the queue and mark
			 * it empty.
			 */
	...
	if (sk->sk_state == TCP_LISTEN) {
		...
		while ((tmpSize < PVTCP_SOCK_SAFE_RCVSIZE) &&
			...
			if (sk->sk_type == SOCK_STREAM) {
				...
				/*
				 * Check if datagram larger than the per cpu
				 * buffer; if so, allocate a large enough
				 * buffer. This should happen quite rarely,
				 * as well-behaved applications don't rely on IP
				...
				if (rc > PVTCP_SOCK_DGRAM_BUF_SIZE) {
					/*
					 * Track large datagram allocations,
					 * whether allocation succeeds or not.
					 * No need for atomic overhead,
					 * approximating is OK.
					 */
					...
					if (!ioBuf) {
						/*
						 * We reset it to the per-cpu
						 * buffer such that we can still
						 * consume the datagram in the
						 * next recvmsg, which will set
						 * MSG_TRUNC so we won't put it
						 * on the channel.
						 */
			...
			/*
			 * If the write failed, we could print a warning.
			 * But if this happened, the comm channel went down.
			 */

			if (inputSize >= coalescingSize) {
				/* We must schedule ourselves back in. */
