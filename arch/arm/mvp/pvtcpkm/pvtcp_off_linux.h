...
PvtcpPutSock(PvtcpSock *pvsk)
{
	...
	/*
	 * Error sockets have no underlying sockets ('sk' == NULL)
	 * so they need to be freed in the 'else' clause.
	 */
...
{
	/*
	 * We must hold the socket before we enqueue it for AIO, such that
	 * it may not be released while in the workqueue.
	 * If CommSvc_ScheduleAIOWork() returned non-zero, it means the socket
	 * had already been enqueued. In that case, we release the hold.
	 * Otherwise, the hold is released by the AIO function
	 * (PvtcpProcessAIO()).
	 * Note that error pv sockets may only originate from synchronized RPCs,
	 * or to be more precise, from PvtcpCreateOp(), and not from IO
	 * processing; this means that they cannot be attempted to be
