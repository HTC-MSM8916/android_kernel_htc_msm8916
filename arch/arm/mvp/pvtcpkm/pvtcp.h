...
typedef struct PvtcpState {
	...
	void *extra;			/* Used by upper layer to extend */
					/*	state as needed. */
	unsigned int mask;		/* Mask used to obfuscate socket */
					/*	pointers. */
} PvtcpState;
...
#define PVTCP_SOCK_COMMON_FIELDS \
	...
	CommOSAtomic deltaAckSize;	/* Recv size updates for peer. */ \
	CommOSAtomic rcvdSize;		/* Bytes received since last ack. */ \
	CommOSAtomic sentSize;		/* Bytes sent; updated by peer. */ \
	CommOSAtomic queueSize;		/* Queue size. */ \
	CommOSList queue;		/* Send queue or recv queue. */ \
