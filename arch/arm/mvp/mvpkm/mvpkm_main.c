...
version_show(struct kobject *kobj,
	     ...
	     char *buf)
{
/* Use a custom string instead of MODULE_ARCH_VERMAGIC
 * to be sure it does not change */
#ifdef CONFIG_ARM64
...
#ifdef CONFIG_COMPAT /* to support IOCTL call from Aarch32 while kernel is Aarch64 */
 ...
 * those of host user/kernel.
 *
 * @param[out] attribL3D when setting up the LPV64 monitor a template L3D
 *                       containing cacheability attributes {S, TEX, CB} used by
 *                       host kernel for normal memory mappings. These may be
 *                       used directly for monitor/guest mappings, since both
 *                       worlds share a common {TRE, MAIR}.
 * @param[out] attribMAN when setting up TTBR0 in the LPV monitor and the page
 *                       tables for the HW monitor this provides the attributes
 *                       in the generic ARM_MemAttrNormal form, suitable for
 *                       configuring TTBR0 + the monitor and guest's [H]MAIR0
...
DetermineMemAttr64(ARM_ARM64_L3D *attribL3D, ARM_MemAttrNormal *attribMAN)
{
	/*
	 * We use set_pte_ext to sample what {S,TEX,CB} bits Linux is using for
	 * normal kernel/user L3D mappings. These bits should be consistent both
	 * with each other and what we use in the monitor since we share various
	 * pages with both host processes, the kernel module and monitor, and
	 * the ARM ARM requires that synonyms have the same cacheability
	 * attributes, see end of B2.9 of ARM DDI 0487A.b
	 */
	HKVA hkva = __get_free_pages(GFP_KERNEL, 0);
		   ...
		   unsigned long arg)
{
	...
	case MVPKM_CPU_INFO64: {
		...
		uint64 mpidr;
		uint64 mTcrEl1;
		// Let's check that Host is only supporting 40 bits
		// The Monitor MMU supports no more than 40 bits physical addresses
 ...
 * @param[in] nrPages number of pages to be allocated
	   ...
	   HkvaMapInfo *mapInfo)
{
	...
	pgprot_t prot;
	int retval;
	int allocateCount = nrPages + 1; /* extra page for alignment */
	int pageIndex = 0;
...
/**
 * @brief Save switching times to world switch page
 * @param wsp World Switch Page
 *
 ...
 * the formula:
 *
 *   now = wsp->switchedAt64 +
 *         (TSC_READ() - wsp->switchedAtTSC) * TSC_TO_RATE64
 *
 * Monotonic boot clock is used to be independant of real time
 * clock changes and to account for host sleep time.
 */
static void
SetSwitchTimes(WorldSwitchPageType *wsp)
{
	...
	/*
	 * Read the timestamp counter *immediately after* monotonic clock
	 * as that will give the most consistent offset between
	 * reading the monotonic clock from get_monotonic_boottime() and
...
//			pmnc = ARM_ReadSystemRegister(ARM_MVP_PERF_PMNC);
...
//				pmnc &= ~ARM_PMNC_D;
//				ARM_WriteSystemRegister(ARM_MVP_PERF_PMNC, pmnc);
//			}
//
//			/* make sure that the CCNT is enabled */
//			pmcnt = ARM_ReadSystemRegister(ARM_MVP_PERF_PMCNT);
//			if ((pmcnt & ARM_PMCNT_C) != ARM_PMCNT_C) {
//				pmcnt |= ARM_PMCNT_C;
//				ARM_WriteSystemRegister(ARM_MVP_PERF_PMCNT, pmcnt);
//			}
//		}
//
		SetSwitchTimes(wsp);
