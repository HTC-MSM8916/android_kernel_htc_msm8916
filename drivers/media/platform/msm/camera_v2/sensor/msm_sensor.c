...
#ifdef CONFIG_LC_S5K5E2_OTP
	if((rc == 0) && (s_ctrl->sensordata->sensor_name != NULL) && ((strcmp(s_ctrl->sensordata->sensor_name,"s5k5e2") == 0) || (strcmp(s_ctrl->sensordata->sensor_name,"s5k5e2_truly")) == 0))
	{
		pr_err("%s:g_s5k5e2_otp_module_id = %x\n", __func__,g_s5k5e2_otp_module_id);
		if((strcmp(s_ctrl->sensordata->sensor_name,"s5k5e2") == 0) && (g_s5k5e2_otp_module_id == 0x7))
		{
			pr_err("%s:it is ofilm s5k5e2\n", __func__);
		}
		else if((strcmp(s_ctrl->sensordata->sensor_name,"s5k5e2_truly") == 0) && (g_s5k5e2_otp_module_id == 0x2))
		{
			pr_err("%s:it is truly s5k5e2\n", __func__);
		}
		else if(g_s5k5e2_otp_module_id == 0x0)
		{
			pr_err("%s:it is unknown s5k5e2\n", __func__);
		}
		else
		{
			pr_err("%s:it is not support s5k5e2\n", __func__);
			rc = -ENODEV;
		}
	}
#endif
