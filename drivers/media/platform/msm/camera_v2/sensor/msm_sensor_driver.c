...
typedef enum {
	GPIO_NODE_DEFAULT,
	GPIO_NODE_NEW_A31_DTUL,
	GPIO_NODE_NEW_A31_UL,
	GPIO_NODE_TYPE_MAX
} GPIO_NODE_TYPE;

static char* getNodeName(GPIO_NODE_TYPE g){
	switch(g){
	case GPIO_NODE_DEFAULT:
		return "gpios";
	case GPIO_NODE_NEW_A31_DTUL:
		return "gpios_a31dtul";
	case GPIO_NODE_NEW_A31_UL:
		return "gpios_a31ul";
	default:
		return 0;
	}
...
static ssize_t sensor_vendor_show_imx219(struct device *dev,
...
static struct device_attribute dev_attr_sensor_imx219 =
	__ATTR(sensor, 0444, sensor_vendor_show_imx219, NULL);

static const char *s5k4h5Vendor = "Samsung";
static const char *s5k4h5NAME = "s5k4h5";
static const char *s5k4h5Size = "8.0M";
static ssize_t sensor_vendor_show_s5k4h5(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
...
static struct device_attribute dev_attr_sensor_s5k4h5 =
	__ATTR(sensor, 0444, sensor_vendor_show_s5k4h5, NULL);

static const char *s5k5eVendor = "Samsung";
static const char *s5k5eNAME = "s5k5e";
static const char *s5k5eSize = "5.0M";
static ssize_t sensor_vendor_show_s5k5e(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
...
static struct device_attribute dev_attr_sensor_s5k5e =
	__ATTR(sensor, 0444, sensor_vendor_show_s5k5e, NULL);

static void msm_sensor_create_node(const char* p_sensor_name, int camera_id)
{
    int ret;
    struct kobject *android_sensor_node;
    if(NULL == p_sensor_name){
        printk("%s: Error sensor name \n", __func__);
        return;
    }

    if(0 == camera_id)
        android_sensor_node = kobject_create_and_add("android_camera", NULL);
    else
         android_sensor_node = kobject_create_and_add("android_camera2", NULL);
    if(NULL == android_sensor_node) {
        printk("%s: Create file node error", __func__);
        return;
    }

    if(strcmp(p_sensor_name, "imx219") == 0)
         ret = sysfs_create_file(android_sensor_node, &dev_attr_sensor_imx219.attr);
    else if(strcmp(p_sensor_name, "s5k4h5_8916") == 0)
         ret = sysfs_create_file(android_sensor_node, &dev_attr_sensor_s5k4h5.attr);
    else if(strcmp(p_sensor_name, "s5k5e2_front") == 0)
         ret = sysfs_create_file(android_sensor_node, &dev_attr_sensor_s5k5e.attr);
    else {
        printk("%s: Unknow sensor name %s \n", __func__, p_sensor_name);
        return;
    }

    if (ret) {
        printk("%s: sysfs_create_file return failed\n", __func__);
        kobject_del(android_sensor_node);
        return;
    }
    printk("%s: Create %s mode success \n", __func__, p_sensor_name);
    return;
}
...
int32_t msm_sensor_driver_probe(void *setting)
{
	...
	s_ctrl->func_tbl->sensor_power_down(s_ctrl);
      ...
      msm_sensor_create_node(slave_info->sensor_name, slave_info->camera_id);
	...
	struct device_node *of_node)
{
	...
	GPIO_NODE_TYPE gpiotype = GPIO_NODE_DEFAULT;
	...
	printk("of_board_is_a31dtul %d, of_machine_pcbid %d\n", of_board_is_a31dtul(), of_machine_pcbid());
	if (of_board_is_a31dtul() && of_machine_pcbid() > 4 ){
		gpiotype = GPIO_NODE_NEW_A31_DTUL;
	}
	if (of_board_is_a31ul() && of_machine_pcbid() >= 4 && of_machine_pcbid() != 5) { 
		gpiotype = GPIO_NODE_NEW_A31_UL;
	}
	gpio_array_size = of_gpio_named_count(of_node, getNodeName(gpiotype));
	printk("<4>gpio count %d\n", gpio_array_size);
	if (!gpio_array_size)
		return 0;
	...
	for (i = 0; i < gpio_array_size; i++) {
		gpio_array[i] = of_get_named_gpio_flags(of_node, getNodeName(gpiotype), i, NULL);
		printk("gpio_array[%d] = %d\n", i, gpio_array[i]);
	}
