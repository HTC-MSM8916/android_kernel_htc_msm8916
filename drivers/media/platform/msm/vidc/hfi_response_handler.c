		...
		struct hfi_msg_session_start_done_packet *pkt)
{
	...
	if (!pkt || pkt->size !=
		...
		dprintk(VIDC_ERR,
				...
				/* HTC_START: Fix klocwork Issue 2281 */
				pkt? pkt->size : 0);
				/* HTC_END */
		return;
	}
		...
		struct hfi_msg_session_stop_done_packet *pkt)
{
	...
	if (!pkt || pkt->size !=
		...
		dprintk(VIDC_ERR,
				...
				/* HTC_START: Fix klocwork Issue 2282 */
				pkt? pkt->size : 0);
				/* HTC_END */
		return;
	}
		...
		struct hfi_msg_session_release_resources_done_packet *pkt)
{
	...
	if (!pkt || pkt->size !=
		...
		dprintk(VIDC_ERR,
				...
				/* HTC_START: Fix klocwork Issue 2283 */
				pkt? pkt->size : 0);
				/* HTC_END */
		return;
	}
		...
		struct hfi_msg_sys_session_end_done_packet *pkt)
{
	...
	if (!pkt || pkt->size !=
		...
		dprintk(VIDC_ERR,
				...
				/* HTC_START: Fix klocwork Issue 2285 */
				pkt? pkt->size : 0);
				/* HTC_END */
		return;
	}
		...
		struct list_head *sessions, struct mutex *session_lock)
{
	...
	if (!callback || !msg_hdr || msg_hdr->size < VIDC_IFACEQ_MIN_PKT_SIZE) {
		dprintk(VIDC_ERR,
				...
				/* HTC_START: Fix klocwork Issue 2279 */
