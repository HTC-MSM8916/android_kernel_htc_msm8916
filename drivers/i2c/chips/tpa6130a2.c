...
void tpa6130a2_channel_enable(int channel, int enable)
{
	unsigned char val;
	int ret=0;
	pr_info("%s : enable=%d\n", __func__, enable);
	   ...
	   unsigned long arg)
{
	...
	int rc = 0, modeid = 0, i = 0, ret = 0;
	int j = 0, val_current = 0, val_new = 0;
	const int sleepMs = 1;
	struct tpa6130a2_data *chipdata = NULL;
	chipdata = i2c_get_clientdata(tpa6130a2_client);
			...
			for (i = 0; i < tpa6130_config_data.cmd_data[modeid].config.reg_len; i++) {
				...
				if (chipdata->power_state != 0) {
					if (val_new != val_current) {
						pr_info("%s: tap6130 ramping, val: %#x -> %#x ++\n", __func__, val_current, val_new);
						if (val_new > val_current) {
							for (j = val_current+1; j <= val_new; j++) {
								msleep(sleepMs);
