...
#endif
        printf("wlan: " MAC_ADDRESS_STR "(encryption) connected to " MAC_ADDRESS_STR "\n",
                ~(pAdapter->macAddressCurrent.bytes)[0]&0xff,~(pAdapter->macAddressCurrent.bytes)[1]&0xff,~(pAdapter->macAddressCurrent.bytes)[2]&0xff,
				~(pAdapter->macAddressCurrent.bytes)[3]&0xff,~(pAdapter->macAddressCurrent.bytes)[4]&0xff,~(pAdapter->macAddressCurrent.bytes)[5]&0xff,
