...
{	
	...
	{
		ERR("ctp_chip_self_test this ctp is not support self test func \n");
		return 0;
	}
	else
	{
		DBG("ctp_chip_self_test call pointer func to test \n");
...
{
	int val=0x00;
	DBG("ctp_selftest_workqueue_func is_in_self_test=%d,retry_count=%d\n",is_in_self_test,retry_count);
	val = ctp_chip_self_test();
	if(val == 0)
	{
		strcpy(ft_ctp_selftest_status,"PASS");
		is_in_self_test = 0;
		DBG("ctp_selftest_workqueue_func self test success \n");
	}
	else
	{
		retry_count--;
		if(retry_count > 0)
		{
			DBG("ctp_selftest_workqueue_func self test retry retry_count=%d\n",retry_count);
			queue_work(ctp_selftest_workqueue, &ctp_selftest_work);
		}
		else
		{
			strcpy(ft_ctp_selftest_status,"FAIL");
			ERR("ctp_selftest_workqueue_func self test failed\n");
			is_in_self_test = 0;
		}
...
static int  ctp_selftest_init(void)
{
	...
	{
		ERR("create_proc_entry g_ctp_selftest_proc failed\n");
	}
	else
	{
		DBG("create_proc_entry g_ctp_selftest_proc success\n");
	}
#else
