...
#define FTS_DBG
#ifdef FTS_DBG
#define DBG(fmt, args...) printk("[TP]" fmt, ## args)
#else
#define DBG(fmt, args...) do{}while(0)
#endif
#define ERR(x...) printk(KERN_ERR "[TP][ERROR] " x)
...
{
	#include "FT5436I_HTC_A31_YS0x5f.h"
};

static unsigned char CTPM_FW_BOYI[] =
{
	#include "FT5436I_HTC_A31_BY0xa6.h"
};
...
{
	DBG("fts_ctpm_get_i_file_ver fts_fw_id= 0x%x\n", fts_fw_id);
...
int fts_ctpm_auto_upgrade(struct i2c_client * client)
{
	...
	{
		DBG("fts_ctpm_auto_upgrade firmware is not match the tp\n");
	  	return 1;
	}

	DBG("uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n", uc_tp_fm_ver, uc_host_fm_ver);
...
#endif
	{
		...
		if (i_ret == 0)
		{
	    	msleep(300);
	    	uc_host_fm_ver = fts_ctpm_get_i_file_ver();
			DBG("upgrade to new version 0x%x\n", uc_host_fm_ver);
		}
		else
		{
			DBG("upgrade failed ret=%d.\n", i_ret);
			return -EIO;
		}
	}
...
int ft5x36_read_project_code(struct i2c_client * client, char * pProjectCode) {
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		{
			DBG(" Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0],reg_val[1]);
	    	break;
		}
		else
		{
			ERR(" Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0],reg_val[1]);
	    	continue;
		}
	}
...
int  fts_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	if(reg_val[0] != bt_ecc)
	{
		ERR("--ecc error! FW=%02x bt_ecc=%02x\n", reg_val[0], bt_ecc);
	    	return -EIO;
	}
...
static int ft5x0x_GetFirmwareSize(char * firmware_name)
{
	...
	if(IS_ERR(pfile)){
		ERR("error occured while opening file %s.\n", filepath);
		return -EIO;
		}
...
int fts_ctpm_fw_upgrade_with_app_file(struct i2c_client * client, char * firmware_name)
{
  	u8*     pbt_buf = NULL;
   	...
   	if(fwsize <= 0)
   	{
		ERR("%s ERROR:Get firmware size failed\n", __FUNCTION__);
		return -EIO;
   	}
	...
	if(ft5x0x_ReadFirmware(firmware_name, pbt_buf))
    	{
			ERR("%s() - ERROR: request_firmware failed\n", __FUNCTION__);
        	kfree(pbt_buf);
		return -EIO;
    	}
		...
		&& (pbt_buf[fwsize-3]^pbt_buf[fwsize-4])==0xFF)
	{
		...
		i_ret =  fts_ctpm_fw_upgrade(client, pbt_buf, fwsize);
   		if (i_ret != 0)
   		{
			ERR("%s() - ERROR:[FTS] upgrade failed i_ret = %d.\n",__FUNCTION__,  i_ret);
   		}
  	 	else
   		{
...
u8 ft5x0x_enter_work(struct i2c_client *client)
{
	u8 regval;
	ft5x0x_write_reg(client, 0x00, 0x00);
	msleep(100);

	if(ft5x0x_read_reg(client, 0x00, &regval)<0)
		ERR("%s ERROR: could not read register\n", __FUNCTION__);
	else
	{
		if((regval & 0x70) != 0x00)
		{
			ERR("%s() - ERROR: The Touch Panel was not put in Work Mode. The Device Mode register contains 0x%02X\n", __FUNCTION__, regval);
			return -1;
		}
	}
 ...
 u8 ft5x0x_enter_factory(struct i2c_client *client)
{
	u8 regval;
	...
	else
	{
		if((regval & 0x70) != 0x40)
		{
			ERR("%s() - ERROR: The Touch Panel was not put in Factory Mode. The Device Mode register contains 0x%02X\n", __FUNCTION__, regval);
			return -1;
		}
	}
	return 0;
...
int fts_Get_RawData(struct i2c_client *client, uint16_t RawData[][FT5x0x_RX_NUM], int step)
{
	int retval  = 0;
    int i       = 0;
	...
	if(ft5x0x_enter_factory(client)<0)
	{
		ERR("%s ERROR: could not enter factory mode", __FUNCTION__);
		retval = -1;
		goto error_return;
	}
	...
	for(i = 0; i < 3; i++){
		...
		{
			ERR("%s %d ERROR: could not read register 0x00", __FUNCTION__, __LINE__);
			retval = -1;
			goto error_return;
		}
		devmode |= 0x80;
		...
		{
			ERR("%s %d ERROR: could not scan", __FUNCTION__, __LINE__);
			retval = -1;
			goto error_return;
		}
	}
	...
	for(rownum=0; rownum<FT5x0x_TX_NUM; rownum++)
	{
		...
		memset(read_buffer_msg, 0x00, 8);
		if(ft5x0x_write_reg(client, 0x01, rownum)<0)
		{
			ERR("%s ERROR:could not write rownum\n", __FUNCTION__);
			retval = -1;
			goto error_return;
		}
		msleep(1);
		...
		if (retval < 0)
		{
			ERR("%s ERROR:Could not read row %u raw data", __FUNCTION__, rownum);
			retval = -1;
			goto error_return;
		}
...
error_return:
	if(ft5x0x_enter_work(client)<0)
	{
		ERR("%s ERROR:could not enter work mode ", __FUNCTION__);
		retval = -1;
	}
	return retval;
}
...
int fts_Get_RawData_lc(struct i2c_client *client, uint16_t RawData[][FT5x0x_RX_NUM], int step)
{
	int retval  = 0;
    int i       = 0;
						...
						const char *buf, size_t count)
{
	...
	if(num_read_chars!=2)
	{
		if(num_read_chars!=4)
		{
			ERR("please input 2 or 4 character\n");
			goto error_return;
		}
	}
	...
	if (0 != retval)
	{
		ERR("%s() - ERROR: Could not convert the given input to a number. The given input was: \"%s\"\n", __FUNCTION__, buf);
    	goto error_return;
	}
	...
	if(2 == num_read_chars)
	{
		...
		if(ft5x0x_read_reg(client, regaddr, &regvalue) < 0)
			ERR("Could not read the register(0x%02x)\n", regaddr);
		else
			ERR("the register(0x%02x) is 0x%02x\n", regaddr, regvalue);
	}
	else
	{
		regaddr = wmreg>>8;
		...
		if(ft5x0x_write_reg(client, regaddr, regvalue)<0)
			ERR("Could not write the register(0x%02x)\n", regaddr);
		else
			DBG("Write 0x%02x into register(0x%02x) successful\n", regvalue, regaddr);
	}
error_return:
	mutex_unlock(&g_device_mutex);
						...
						const char *buf, size_t count)
{
	...
	if (i_ret == 0)
	{
	    msleep(300);
	    uc_host_fm_ver = fts_ctpm_get_i_file_ver();
	    DBG("%s upgrade to new version 0x%x\n", __FUNCTION__, uc_host_fm_ver);
	}
	else
	{
	    ERR("%s ERROR:[FTS] upgrade failed ret=%d.\n", __FUNCTION__, i_ret);
	}
	enable_irq(client->irq);
...
static int ft5x0x_get_testparam_from_ini(char *config_name)
{
	...
	if (ft5x0x_ReadInIData(config_name, filedata)) {
		...
		DBG("ft5x0x_ReadInIData successful\n");
	}
						...
						const char *buf, size_t count)
{
	...
	if(ft5x0x_get_testparam_from_ini(cfgname) <0)
		DBG("get testparam from ini failure\n");
	else {
		#if 1
		if(true == StartTestTP())
			DBG("tp test pass\n");
...
int ft5x0x_create_sysfs(struct i2c_client * client)
{
	...
	err = sysfs_create_group(&client->dev.kobj, &ft5x0x_attribute_group);
   	if (0 != err)
  	{
		ERR("%s() - ERROR: sysfs_create_group() failed.error code: %d\n", __FUNCTION__, err);
   	...
   	else
	{
		...
		DBG("ft5x0x:%s() - sysfs_create_group() succeeded. \n", __FUNCTION__);
    }
	return err;
}
...
{
	ft5x0x_proc_entry = proc_create_data(PROC_NAME, 0660, NULL, &ft5x0x_proc_fops, NULL);
	if (IS_ERR_OR_NULL(ft5x0x_proc_entry))
	{
		ERR("add /proc/msg-ito-test/debug error \n");
	}
	return 0;
}
