		...
		unsigned char reg_addr, unsigned char *data)
{
	...
	for (loop_i = 0; loop_i < I2C_RETRY_COUNT; loop_i++) {
		...
		if (dummy < 0) {
			printk("[GSNR][bma2x2_warning] %s i2c err, register = 0x%x,data = 0x%x\n",__func__,reg_addr,dummy & 0x000000ff);
			msleep(10);
		}
		else {
			break;
	...
	if (loop_i >= I2C_RETRY_COUNT) {
		printk("[GSNR][bma2x2_err] %s retry over %d\n",__func__,I2C_RETRY_COUNT);
		return -EIO;
	}
		...
		unsigned char reg_addr, unsigned char *data, unsigned char len)
{
	...
	for (loop_i = 0; loop_i < I2C_RETRY_COUNT; loop_i++) {
		...
		if (dummy < 0) {
			printk("[GSNR][bma2x2_warning] %s i2c err, register = 0x%x\n",__func__,reg_addr);
			msleep(10);
		}
		else
			break;
		...
		const char *buf, size_t count)
{
	...
	if ((bma2x2->pdata->gs_kvalue & (0xFF << 24)) != (0x67 << 24)) {
		bma2x2->offset_buf[0] = 0;
		bma2x2->offset_buf[1] = 0;
		bma2x2->offset_buf[2] = 0;
	} else if ((bma2x2->pdata->gs_kvalue_msb & (0xFF << 24)) == (0x67 << 24)) {
		bma2x2->offset_buf[0] = (bma2x2->offset_buf_msb[0] << 8) | ((bma2x2->pdata->gs_kvalue >> 16) & 0xFF);
		bma2x2->offset_buf[1] = (bma2x2->offset_buf_msb[1] << 8) | ((bma2x2->pdata->gs_kvalue >>  8) & 0xFF);
		bma2x2->offset_buf[2] = (bma2x2->offset_buf_msb[2] << 8) | (bma2x2->pdata->gs_kvalue        & 0xFF);

		for (i = 0; i < 3; i++) {
			if (bma2x2->offset_buf[i] > 32767) {
				bma2x2->offset_buf[i] =
					bma2x2->offset_buf[i] - 65536;
			}
		}
	} else {
...
static ssize_t bma2x2_set_k_value_msb_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bma2x2_data *bma2x2 = i2c_get_clientdata(client);
	...
	return sprintf(buf, "gs_kvalue_msb = 0x%x\n", bma2x2->pdata->gs_kvalue_msb);
}

static ssize_t bma2x2_set_k_value_msb_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	...
	bma2x2->pdata->gs_kvalue_msb = simple_strtoul(buf, NULL, 10);
	D("%s: bma2x2->pdata->gs_kvalue_msb = 0x%x\n", __func__, bma2x2->pdata->gs_kvalue_msb);

	if ((bma2x2->pdata->gs_kvalue_msb & (0xFF << 24)) != (0x67 << 24)) {
		bma2x2->offset_buf_msb[0] = 0;
		bma2x2->offset_buf_msb[1] = 0;
		bma2x2->offset_buf_msb[2] = 0;
	} else {
		bma2x2->offset_buf_msb[0] = (bma2x2->pdata->gs_kvalue_msb >> 16) & 0xFF;
		bma2x2->offset_buf_msb[1] = (bma2x2->pdata->gs_kvalue_msb >>  8) & 0xFF;
		bma2x2->offset_buf_msb[2] =  bma2x2->pdata->gs_kvalue_msb        & 0xFF;
	}
	return count;
}
...
static DEVICE_ATTR(set_k_value_msb, S_IRUSR|S_IRGRP|S_IWUSR,
		bma2x2_set_k_value_msb_show, bma2x2_set_k_value_msb_store);
...
static int bma2x2_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	bma2x2_set_enable(&client->dev, 0);
	I("%s:range=%d,bw=%d\n",__func__,data->range,data->bandwidth);
	return 0;
}
